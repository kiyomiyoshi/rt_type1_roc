...
f <- dir("ReplicationApril2023AllData", ".*csv$", full.names = TRUE) 
df <- c()
for (i in 1:length(f)) {
  d <- fread(f[i], header = TRUE)
  d$Subj_idx <- i
  df <- rbind(df, d)
}
...
for (i in unique(df$Subj_idx)) {
  ...
  result_vec <- as.numeric(unlist(result))
  uvsdt_rt <- rbind(uvsdt_rt, c(result_vec, dp, i))
  result2_vec <- as.numeric(unlist(result2))
...
ggplot(df_wide) + 
  ...
  ggtitle("Dijkstra_2024_Expt2_3") + 
  theme_minimal() +
  theme(plot.title = element_text(size = 8),
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> sigma_Dijkstra_2024_Expt2_3
...
ggplot(df_wide) + 
  ...
  theme(plot.title = element_text(size = 8),
        ...
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> mu_Dijkstra_2024_Expt2_3
...
ggplot(df_wide) + 
  ...
  theme(plot.title = element_text(size = 8),
        ...
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> da_Dijkstra_2024_Expt2_3

# exclusion
length(unique(df$Subj_idx)) # 127
sum(dp >= 0) # 118
nrow(df_wide) # 112