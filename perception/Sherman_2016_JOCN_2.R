...
df <- fread("data_Sherman_2016_JOCN.csv", header = T) # sub 18
df <- subset(df, df$Condition == "2") # Full attention, expect target present
df %>%
  group_by(Subj_idx) %>%
  summarise(n = n()) %>%
  print(n = 99999)
...
for (i in unique(df$Subj_idx)) {
  ...
  nr_s1 <- c(sum(d$Stimulus == "1" & d$Response == "0" & d$Confidence == 4, na.rm = TRUE),
  ...
  nr_s2 <- c(sum(d$Stimulus == "0" & d$Response == "0" & d$Confidence == 4, na.rm = TRUE),
...
ggplot(df_wide) + 
  ...
  ggtitle("Sherman_2016_JOCN_2") + 
  theme_minimal() +
  theme(plot.title = element_text(size = 8),
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> sigma_Sherman_2016_JOCN_2
...
ggplot(df_wide) + 
  ...
  theme(plot.title = element_text(size = 8),
        ...
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> mu_Sherman_2016_JOCN_2
...
ggplot(df_wide) + 
  ...
  theme(plot.title = element_text(size = 8),
        ...
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> da_Sherman_2016_JOCN_2

# exclusion
length(unique(df$Subj_idx)) # 18
sum(dp >= 0) # 18
nrow(df_wide) # 17