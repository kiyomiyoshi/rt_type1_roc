...
df <- fread("Exp2rows.csv", header = T) # 32 trials / sub
df <- select(df, trial_type, trial_index, subject_identifier, first_target, second_target, practice_loops, 
             response, RT, max_p, hide_proportion, test_part, present, target, correct_response, correct, confidence)
df <- na.omit(df)
df <- distinct(df, subject_identifier, trial_index, .keep_all = TRUE)
length(unique(df$subject_identifier)) # only 234 should be included in the analysis
df <- subset(df, df$practice_loops == 0)
df <- subset(df, df$test_part != "practice")
nrow(df)
...
dp <- c()
roc_conf <- c()
for (i in unique(df$subject_identifier)) {
  
  d <- subset(df, df$subject_identifier == i)
  d$Conf_bin <- ntile(d$confidence, 3)
  
  nr_s1 <- c(sum(d$present == "0" & d$correct == "0" & d$Conf_bin == 3, na.rm = TRUE),
             sum(d$present == "0" & d$correct == "0" & d$Conf_bin == 2, na.rm = TRUE),
             sum(d$present == "0" & d$correct == "0" & d$Conf_bin == 1, na.rm = TRUE),
             sum(d$present == "0" & d$correct == "1" & d$Conf_bin == 1, na.rm = TRUE),
             sum(d$present == "0" & d$correct == "1" & d$Conf_bin == 2, na.rm = TRUE),
             sum(d$present == "0" & d$correct == "1" & d$Conf_bin == 3, na.rm = TRUE))
  
  nr_s2 <- c(sum(d$present == "1" & d$correct == "1" & d$Conf_bin == 3, na.rm = TRUE),
             sum(d$present == "1" & d$correct == "1" & d$Conf_bin == 2, na.rm = TRUE),
             sum(d$present == "1" & d$correct == "1" & d$Conf_bin == 1, na.rm = TRUE),
             sum(d$present == "1" & d$correct == "0" & d$Conf_bin == 1, na.rm = TRUE),
             sum(d$present == "1" & d$correct == "0" & d$Conf_bin == 2, na.rm = TRUE),
             sum(d$present == "1" & d$correct == "0" & d$Conf_bin == 3, na.rm = TRUE))
...
for (i in unique(df$subject_identifier)) {
  ...
  d <- subset(df, df$subject_identifier == i)
  d$RT_bin <- ntile(d$RT, 3)
  nr_s1 <- c(sum(d$present == "0" & d$correct == "0" & d$RT_bin == 1, na.rm = TRUE),
             sum(d$present == "0" & d$correct == "0" & d$RT_bin == 2, na.rm = TRUE),
             sum(d$present == "0" & d$correct == "0" & d$RT_bin == 3, na.rm = TRUE),
             sum(d$present == "0" & d$correct == "1" & d$RT_bin == 3, na.rm = TRUE),
             sum(d$present == "0" & d$correct == "1" & d$RT_bin == 2, na.rm = TRUE),
             sum(d$present == "0" & d$correct == "1" & d$RT_bin == 1, na.rm = TRUE))
  
  nr_s2 <- c(sum(d$present == "1" & d$correct == "1" & d$RT_bin == 1, na.rm = TRUE),
             sum(d$present == "1" & d$correct == "1" & d$RT_bin == 2, na.rm = TRUE),
             sum(d$present == "1" & d$correct == "1" & d$RT_bin == 3, na.rm = TRUE),
             sum(d$present == "1" & d$correct == "0" & d$RT_bin == 3, na.rm = TRUE),
             sum(d$present == "1" & d$correct == "0" & d$RT_bin == 2, na.rm = TRUE),
             sum(d$present == "1" & d$correct == "0" & d$RT_bin == 1, na.rm = TRUE))
...
ggplot(df_wide) + 
  ...
  ggtitle("Mazor_2025_Expt2") + 
  theme_minimal() +
  theme(plot.title = element_text(size = 8),
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> sigma_Mazor_2025_Expt2
...
ggplot(df_wide) + 
  ...
  theme(plot.title = element_text(size = 8),
        ...
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> mu_Mazor_2025_Expt2
