...
df <- c()
f1 <- dir("CollectionSept2021", ".*csv$", full.names = TRUE) 
for (i in 1:length(f1)) {
  d <- fread(f1[i], header = TRUE)
  d$Subj_idx <- i
  df <- rbind(df, d)
}

f2 <- dir("CollectionNov2022", ".*csv$", full.names = TRUE) 
for (i in 1:length(f2)) {
  d <- fread(f2[i], header = TRUE)
  d$Subj_idx <- i + 65
  df <- rbind(df, d)
}

df <- subset(df, df$ConditionID == 3) # 96 trials / sub
df %>%
  group_by(Subj_idx) %>%
  summarise(n = n()) %>%
  print(n = 99999)
...
for (i in unique(df$Subj_idx)) {
  ...
  nr_s1 <- c(sum(d$StimPresent == "0" & d$Correct == "0" & d$Conf_bin == 3, na.rm = TRUE),
             sum(d$StimPresent == "0" & d$Correct == "0" & d$Conf_bin == 2, na.rm = TRUE),
             sum(d$StimPresent == "0" & d$Correct == "0" & d$Conf_bin == 1, na.rm = TRUE),
             sum(d$StimPresent == "0" & d$Correct == "1" & d$Conf_bin == 1, na.rm = TRUE),
             sum(d$StimPresent == "0" & d$Correct == "1" & d$Conf_bin == 2, na.rm = TRUE),
             sum(d$StimPresent == "0" & d$Correct == "1" & d$Conf_bin == 3, na.rm = TRUE))
  
  nr_s2 <- c(sum(d$StimPresent == "1" & d$Correct == "1" & d$Conf_bin == 3, na.rm = TRUE),
             sum(d$StimPresent == "1" & d$Correct == "1" & d$Conf_bin == 2, na.rm = TRUE),
             sum(d$StimPresent == "1" & d$Correct == "1" & d$Conf_bin == 1, na.rm = TRUE),
             sum(d$StimPresent == "1" & d$Correct == "0" & d$Conf_bin == 1, na.rm = TRUE),
             sum(d$StimPresent == "1" & d$Correct == "0" & d$Conf_bin == 2, na.rm = TRUE),
             sum(d$StimPresent == "1" & d$Correct == "0" & d$Conf_bin == 3, na.rm = TRUE))
...
for (i in unique(df$Subj_idx)) {
  ...
  nr_s1 <- c(sum(d$StimPresent == "0" & d$Correct == "0" & d$RT_bin == 1, na.rm = TRUE),
             sum(d$StimPresent == "0" & d$Correct == "0" & d$RT_bin == 2, na.rm = TRUE),
             sum(d$StimPresent == "0" & d$Correct == "0" & d$RT_bin == 3, na.rm = TRUE),
             sum(d$StimPresent == "0" & d$Correct == "1" & d$RT_bin == 3, na.rm = TRUE),
             sum(d$StimPresent == "0" & d$Correct == "1" & d$RT_bin == 2, na.rm = TRUE),
             sum(d$StimPresent == "0" & d$Correct == "1" & d$RT_bin == 1, na.rm = TRUE))
  
  nr_s2 <- c(sum(d$StimPresent == "1" & d$Correct == "1" & d$RT_bin == 1, na.rm = TRUE),
             sum(d$StimPresent == "1" & d$Correct == "1" & d$RT_bin == 2, na.rm = TRUE),
             sum(d$StimPresent == "1" & d$Correct == "1" & d$RT_bin == 3, na.rm = TRUE),
             sum(d$StimPresent == "1" & d$Correct == "0" & d$RT_bin == 3, na.rm = TRUE),
             sum(d$StimPresent == "1" & d$Correct == "0" & d$RT_bin == 2, na.rm = TRUE),
             sum(d$StimPresent == "1" & d$Correct == "0" & d$RT_bin == 1, na.rm = TRUE))
...
ggplot(df_wide) + 
  ...
  ggtitle("Dijkstra_2024_Expt1_3") + 
  theme_minimal() +
  theme(plot.title = element_text(size = 8),
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> sigma_Dijkstra_2024_Expt1_3
...
ggplot(df_wide) + 
  ...
  theme(plot.title = element_text(size = 8),
        ...
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> mu_Dijkstra_2024_Expt1_3
...
ggplot(df_wide) + 
  ...
  annotation_custom(
    grob = textGrob(
      label = paste("y =", round(coef(lm(da_rt ~ da_conf, data = df_wide))[2], 2), 
                    "x -", abs(round(coef(lm(da_rt ~ da_conf, data = df_wide))[1], 2)), 
  ...
  theme(plot.title = element_text(size = 8),
        ...
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> da_Dijkstra_2024_Expt1_3