...
df <- fread("data_Kantner_2014_E2.csv", header = T)
df <- subset(df, df$StimulusType == "Face")
length(unique(df$Subj_idx)) # 74
df %>%
  group_by(Subj_idx) %>%
  summarise(n = n()) %>%
  print(n = 99999) # 76
...
for (i in unique(df$Subj_idx)) {
  ...
  nr_s1 <- c(sum(d$Stimulus == "0" & d$Response == "1" & d$Confidence == 3, na.rm = TRUE),
  ...
  nr_s2 <- c(sum(d$Stimulus == "1" & d$Response == "1" & d$Confidence == 3, na.rm = TRUE),
...
ggplot(df_wide) + 
  ...
  ggtitle("Kantner_2014_E2_Face") + 
  theme_minimal() +
  theme(plot.title = element_text(size = 8),
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> sigma_Kantner_2014_E2_Face
...
ggplot(df_wide) + 
  ...
  theme(plot.title = element_text(size = 8),
        ...
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> mu_Kantner_2014_E2_Face
...
ggplot(df_wide) + 
  ...
  theme(plot.title = element_text(size = 8),
        ...
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> da_Kantner_2014_E2_Face