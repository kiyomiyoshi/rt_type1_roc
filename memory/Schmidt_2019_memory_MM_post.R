...
df <- fread("data_Schmidt_2019_memory.csv", header = T)
df <- subset(df, df$Condition == "MM") # first/last 200 trials before/after training
length(unique(df$Subj_idx)) # 14
df %>%
  group_by(Subj_idx) %>%
  summarise(n = n()) %>%
  print(n = 99999) # 200
...
#'# confidence (4 level 0.28, 6 level 0.37)
uvsdt_conf <- c()
meta_conf <- c()
dp <- c()
roc_conf <- c()
for (i in unique(df$Subj_idx)) {
  ...
  d <- d[201:nrow(d), ]
  d$Conf_bin <- ntile(d$Confidence, 6)
  
  nr_s1 <- c(sum(d$Stimulus == "2" & d$Response == "1" & d$Conf_bin == 6, na.rm = TRUE),
             sum(d$Stimulus == "2" & d$Response == "1" & d$Conf_bin == 5, na.rm = TRUE),
             sum(d$Stimulus == "2" & d$Response == "1" & d$Conf_bin == 4, na.rm = TRUE),
             sum(d$Stimulus == "2" & d$Response == "1" & d$Conf_bin == 3, na.rm = TRUE),
             ...
             sum(d$Stimulus == "2" & d$Response == "2" & d$Conf_bin == 4, na.rm = TRUE),
             sum(d$Stimulus == "2" & d$Response == "2" & d$Conf_bin == 5, na.rm = TRUE),
             sum(d$Stimulus == "2" & d$Response == "2" & d$Conf_bin == 6, na.rm = TRUE))
  
  nr_s2 <- c(sum(d$Stimulus == "1" & d$Response == "1" & d$Conf_bin == 6, na.rm = TRUE),
             sum(d$Stimulus == "1" & d$Response == "1" & d$Conf_bin == 5, na.rm = TRUE),
             sum(d$Stimulus == "1" & d$Response == "1" & d$Conf_bin == 4, na.rm = TRUE),
             sum(d$Stimulus == "1" & d$Response == "1" & d$Conf_bin == 3, na.rm = TRUE),
             ...
             sum(d$Stimulus == "1" & d$Response == "2" & d$Conf_bin == 4, na.rm = TRUE),
             sum(d$Stimulus == "1" & d$Response == "2" & d$Conf_bin == 5, na.rm = TRUE),
             sum(d$Stimulus == "1" & d$Response == "2" & d$Conf_bin == 6, na.rm = TRUE))
...
ggplot(df_wide) + 
  geom_point(aes(x = sigma_conf, y = sigma_rt), alpha = 0.5) + xlim(0, 3) + ylim(0, 3) +
  ...
  ggtitle("Schmidt_2019_MM_post") + 
  theme_minimal() +
  theme(plot.title = element_text(size = 8),
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> sigma_Schmidt_2019_MM_post

# mu
correlation_mu <- cor(df_wide$mu_conf, df_wide$mu_rt)
ggplot(df_wide) + 
  geom_point(aes(x = mu_conf, y = mu_rt), alpha = 0.5) + xlim(0, 6) + ylim(0, 6) +
  geom_smooth(aes(x = mu_conf, y = mu_rt), method = "lm", se = FALSE, size = 0.7) +
  annotation_custom(
    grob = textGrob(
      label = paste("y =", round(coef(lm(mu_rt ~ mu_conf, data = df_wide))[2], 2), 
                    "x -", abs(round(coef(lm(mu_rt ~ mu_conf, data = df_wide))[1], 2)), 
  ...
  theme(plot.title = element_text(size = 8),
        ...
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> mu_Schmidt_2019_MM_post

# da
correlation_da <- cor(df_wide$da_conf, df_wide$da_rt)
ggplot(df_wide) + 
  geom_point(aes(x = da_conf, y = da_rt), alpha = 0.5) + xlim(0, 3) + ylim(0, 3) +
  ...
  theme(plot.title = element_text(size = 8),
        ...
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> da_Schmidt_2019_MM_post