...
df <- subset(df, df$Condition == "SoB") # first/last 200 trials before/after training
length(unique(df$Subj_idx)) # 13
df %>%
  group_by(Subj_idx) %>%
  summarise(n = n()) %>%
  print(n = 99999) # 200
...
#'# confidence (4 level 0.24, 6 level 0.67)
uvsdt_conf <- c()
meta_conf <- c()
dp <- c()
roc_conf <- c()
...
ggplot(df_wide) + 
  ...
  ggtitle("Schmidt_2019_SoB_pre") + 
  theme_minimal() +
  theme(plot.title = element_text(size = 8),
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> sigma_Schmidt_2019_SoB_pre
...
ggplot(df_wide) + 
  ...
  theme(plot.title = element_text(size = 8),
        ...
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> mu_Schmidt_2019_SoB_pre
...
ggplot(df_wide) + 
  ...
  theme(plot.title = element_text(size = 8),
        ...
        axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 6),
        axis.text.y  = element_text(size = 6)) -> da_Schmidt_2019_SoB_pre